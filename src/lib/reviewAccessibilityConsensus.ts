...
export function consensusFieldStrictYes(
  c: ConsensusForField | null | undefined,
): boolean {
  if (!c) return false;
...
/** Fila de `reviews` usada para consenso (checklist + place_id + source). */
export type PlaceReviewConsensusRow = Record<
  AccessibilityReviewKey,
  boolean | null
> & {
  id?: number;
